...
resource "azurerm_monitor_diagnostic_setting" "sc_diag_enterprise" {
  ...
  name                        = "monitoring"
  target_resource_id          = azurerm_spring_cloud_service.sc_enterprise[0].id
...
data "azurerm_lb" "svc_load_balancer_enterprise" {
...
resource "azurerm_private_dns_a_record" "a_record_enterprise" {
  ...
  records             = [data.azurerm_lb.svc_load_balancer_enterprise[0].frontend_ip_configuration[0].private_ip_address]
}
